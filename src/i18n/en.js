...
export default {
  ...
  ui: {
    ...
    'splash.manageFigures':  'Manage figures…',

    'settings.title':        'Manage figures',
    'settings.hint':         'Upload a .zip containing PNG/WebP/JPG files named after entity slugs (e.g. brute.png, bandit-archer.png). Images are stored only on this device.',
    'settings.upload':       'Upload zip',
    'settings.clear':        'Clear all',
    'settings.clearConfirm': 'Remove all uploaded figures from this device?',
    'settings.cleared':      'All figures cleared from this device.',
    'settings.unpacking':    'Unpacking zip…',
    'settings.imported':     (n) => `Imported ${n} figure${n === 1 ? '' : 's'}`,
    'settings.unknown':      (n) => `${n} unknown slug${n === 1 ? '' : 's'}`,
    'settings.skipped':      (n) => `${n} non-image file${n === 1 ? '' : 's'} skipped`,
    'settings.errorPrefix':  'Failed to load',
    'settings.errorRead':    'Could not read the zip file.',
    'settings.none':         'No figures uploaded on this device yet.',
    'settings.loaded':       (n) => `${n} figure${n === 1 ? '' : 's'} loaded on this device:`,
